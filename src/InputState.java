...
public class InputState {
	...
	private JComboBox<ArtifactState> stateCB;
	...
	public void main() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InputState window = new InputState(artifactList, parent, resultTextArea);
	 ...
	 */
	public InputState(List<Artifact> artifactList, DefaultTableModel parentModel, JTextArea resultTextArea) {
		this.artifactList = artifactList;
	...
	private void initialize() {
		...
		frmAddNewAttribute.addWindowListener(new WindowListener() {
			...
			public void windowClosed(WindowEvent e) {
				if (result != null) {
					...
					resultTextArea.setText(((resultTextArea.getText().equals(""))?"":(resultTextArea.getText()+" & ")) + result[0]);
				}
			}
		...
		artifactCB.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				if (arg0.getStateChange() == ItemEvent.SELECTED) {
					Artifact selectedArtifact = (Artifact) stateCB.getSelectedItem();
					stateCB.removeAllItems();
					for (ArtifactState i : selectedArtifact.getListState()) {
						stateCB.addItem(i);
					}
				}
			}
		...
		stateCB = new JComboBox(artifactList.get(0).getListState().toArray());
		stateCB.setBounds(100, 80, 200, 30);
		frmAddNewAttribute.getContentPane().add(stateCB);

		JButton btnAdd = new JButton("Add");
		btnAdd.setBounds(20, 140, 80, 30);
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String tmp[] = {artifactCB.getSelectedItem()+"Is"+stateCB.getSelectedItem()};
				result = tmp;
		...
		btnCancel.setBounds(100, 140, 80, 30);
